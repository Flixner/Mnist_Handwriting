...
from PyQt5.QtGui import (QPainter, QPen, QPainterPath, QImage, QTransform, QFont, QColor, QKeySequence, QPalette,
                         QFontMetrics)
...
class NumberVectorWidget(ResizeFrame):
    ...
    def __init__(self, image_shape=(16, 16), format=QImage.Format_Grayscale8):
        super().__init__()
        self.image = QImage(*image_shape, format)
    ...
    def paintEvent(self, e):
        ...
        b = self.lineWidth()
        fr.adjust(b, b, -b, -b)
        qp.drawImage(QRect(fr.x(), fr.y(), fr.width(), fr.height()), self.image,
...
class MainWidget(QWidget):
    ...
    def __init__(self, model=None):
        ...
        self.nvw = NumberVectorWidget((28, 28))
...
np.array([  3.28861030e-04,  -5.71444401e-04,   8.44269446e-02,   9.65669462e-02,   1.56398568e-03,  -2.55995098e-03,   4.83468272e-03,   2.12933916e+00]),
    ...
    def updateNumber(self):
        ...
        # -----------------------------------------------------------------Modifizeriter Code
        self.dnw.number = None
        self.dpw.number = None
        #--------------- für google

        #vec = np.array(ptr)
        # image = np.resize(vec, (224, 224, 3))
        ...
        #---------------- für keras mnist model
        vec = np.array(ptr) * (-1.0 / 255.0) + 1.0
        data = np.reshape(vec, (1, len(vec)))
        data = np.array(data, dtype=np.float32)

        if self.model:
            p = self.model.predict_on_batch(data)[0]
            self.dpw.probabilities = p
            self.dnw.number = np.argmax(self.dpw.probabilities)
            if np.max(p) < 0.7:
                self.dnw.number = None
